        ...
        if((*iter).accountHolderName == client.getClientName() && (*iter).accountNumber == accountNumber) {
            if((*iter).hasLoan) {
                ...
                if((*iter).amountRepayed >= (0.6)*(*iter).loanAmount) {
                    cout << "Amount repaid(" << (*iter).amountRepayed << "), greater than 60 percent of the loan amount of " << (*iter).loanAmount << " equalling " << (*iter).loanAmount * 0.6 << endl;
                    if(this->gp2.getManager().agreeForLoan(client, amount)) {
                        ...
                        try {
                            throw ("Manager has not agreed to grant the loan!");
                        }
                        catch (std::exception& e){
                            cout << e.what() << endl;
                            return 1;
                    ...
                    try {
                        throw std::domain_error("Previous loan amount of " + std::to_string((*iter).loanAmount) + " has not been paid, or atleast 60 percent of the loans amount has not been re-paid.");
                    }
                    catch (std::exception& e){
                        cout << e.what() << endl;
                        return 1;
                ...
                if(this->gp2.getManager().agreeForLoan(client, amount)) {
                    ...
                    return 0;
                     ...
                     try {
                         throw std::invalid_argument("Manager retrieval error.");
                     }
                     catch (std::exception& e) {
                         cout << e.what() << endl;
                         return 1;
...
bool Bank::repayLoan(Client &client, int acctn,  int amount) {
    ...
    for(iter = this->bankAccounts.begin(); iter != this->bankAccounts.end(); iter++ ){
        if((*iter).accountHolderName == client.getClientName() && (*iter).accountNumber == acctn) {
            ...
            if((*iter).amountRepayed >= 0.6 * (*iter).loanAmount) {
                cout << "Client " << client.getClientName() << " is eligible for another loan." << endl;
                (*iter).hasLoan = false;
                return 0;
            }
            else {
                ...
                try {
                    throw std::invalid_argument ("Client " + client.getClientName() + " is not YET eligible for another loan.");
                }
                catch(std::exception& e) {
                    cout << e.what() << endl;
                }
